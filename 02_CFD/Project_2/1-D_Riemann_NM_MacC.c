...
struct Riemann_1D_MacC_solver {
    ...
    //  预测步变量, bar表示上加一线，对应ppt里的描述
    double *q1_bar; // 这是预测步量，注意守恒量q1=rho
    double *q2_bar; // 这是预测步量，注意守恒量q2=rho*u
    double *q3_bar; // 这是预测步量，注意守恒量q3=rho*E

    //  矫正步，也即 at the next time step
    double *q1_next;    // 这是矫正步变量，注意守恒量q1=rho
    double *q2_next;    // 这是矫正步变量，注意守恒量q2=rho*u
    double *q3_next;    // 这是矫正步变量，注意守恒量q3=rho*E

    //  当前步和预测步的失通量
    double *f1;         //失通量
    double *f2;         //失通量
    double *f3;         //失通量
    double *f1_bar;     //预测步，失通量
    double *f2_bar;     //预测步，失通量
    double *f3_bar;     //预测步，失通量
